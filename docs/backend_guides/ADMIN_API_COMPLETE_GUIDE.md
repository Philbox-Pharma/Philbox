...
3. [Admin Authentication APIs](#1-admin-authentication-apis)
4. [Branch Management APIs](#2-branch-management-apis)
5. [User Management (Admins & Salespersons)](#3-user-management-apis)
6. [Permissions & RBAC](#4-permissions--rbac-apis)
7. [Customer Management](#5-customer-management-apis)
8. [Doctor Management](#6-doctor-management-apis)
9. [Salesperson Task Management](#7-salesperson-task-management-apis)
10. [Dashboard Analytics](#8-dashboard-analytics-apis)
...
- Most endpoints require session-based authentication (express-session)
- Additional RBAC checks use `roleMiddleware('super_admin')` or `rbacMiddleware('<permission>')`
- Sensitive endpoints are restricted to super-admin role
...
- `account_status` (optional): Filter by status - `active`, `suspended/freezed`, `blocked/removed`
...
- Sets doctor `account_status` to `active`
...
- Keeps doctor `account_status` as `suspended/freezed`
...
- `account_status` (optional): Filter by status - `active`, `suspended`, `blocked`
...
**Fields:** `_id`, `fullName`, `email`, `contactNumber`, `account_status`, `is_Verified`, `created_at`

### Doctor Model

**Fields:** `_id`, `fullName`, `email`, `gender`, `dateOfBirth`, `contactNumber`, `educational_details[]`, `specialization[]`, `experience_details[]`, `license_number`, `affiliated_hospital`, `consultation_type`, `consultation_fee`, `account_status`, `averageRating`, `profile_img_url`, `cover_img_url`, `last_login`, `roleId`, `created_at`

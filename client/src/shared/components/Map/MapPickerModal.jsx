...
import { FaTimes, FaSearch, FaMapMarkerAlt, FaCheck, FaSpinner, FaLocationArrow } from "react-icons/fa";
...
export default function MapPickerModal({ isOpen, onClose, onSelect, initialQuery = "", initialCenter = { lat: 31.5204, lng: 74.3587 } }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTimeout(() => { window.dispatchEvent(new Event('resize')); }, 500); // Fix Leaflet render issue
    }
  }, [isOpen]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="px-6 py-4 bg-[#1a365d] text-white flex justify-between items-center flex-shrink-0">
              ...
              <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-lg"><FaTimes /></button>
            </div>
  ...
  <div className="relative">
                ...
                />
                {searching && <FaSpinner className="absolute right-4 top-3.5 animate-spin text-[#1a365d]" />}

                {showSuggestions && suggestions.length > 0 && (
 <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-2xl border border-gray-200 max-h-60 overflow-y-auto z-[600]">                    {suggestions.map((s, i) => (
                      <div key={i} onClick={() => handleSelect(s)} className="p-3 hover:bg-gray-50 cursor-pointer flex gap-3 border-b last:border-0">
                        <FaLocationArrow className="mt-1 text-gray-400" />
                        <div>
                          <p className="text-sm font-medium text-gray-800">{s.display_name.split(',')[0]}</p>
            ...
            <div className="flex-1 relative bg-gray-100 w-full">
              <MapContainer center={center} zoom={13} style={{ height: "100%", width: "100%" }}>
                ...
                <Marker position={marker} draggable eventHandlers={{ dragend: (e) => setMarker(e.target.getLatLng()) }} />
              </MapContainer>
              ...
              <div className="absolute bottom-6 left-6 right-6 z-[1000]">
                <div className="bg-white p-4 rounded-xl shadow-lg border flex flex-col sm:flex-row justify-between items-center gap-4">
                  <div className="text-sm text-gray-600">
                    <span className="font-bold block text-xs text-gray-400 uppercase">Selected Coordinates</span>
                    {marker.lat.toFixed(5)}, {marker.lng.toFixed(5)}
                  </div>
                  <button onClick={confirmLocation} className="px-6 py-2.5 bg-[#d69e2e] text-white rounded-lg hover:bg-[#b8860b] flex items-center gap-2 font-medium w-full sm:w-auto justify-center">

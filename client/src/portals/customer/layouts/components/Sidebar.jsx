  ...
  return (
    <>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50">
        <div className="flex justify-between items-center py-2 px-0.5 sm:px-4">
          {navItems.slice(0, 5).map(item => (
            <Link
              key={item.path}
              to={item.path}
              className={`flex flex-col items-center flex-1 min-w-0 px-0.5 sm:px-1 gap-1 transition-colors ${
                isActive(item.path)
                  ? 'text-blue-600'
                  : 'text-gray-500 hover:text-gray-700'
              }`}
            >
              <item.icon className="text-base sm:text-xl" />
              <span className="text-[9px] sm:text-xs truncate w-full text-center">
                {item.label}
              </span>
            </Link>
          ))}

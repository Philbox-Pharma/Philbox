...
export default function Sidebar() {
    ...
    return (
        <>
            {/* Desktop Sidebar */}
            <aside
                className={`hidden lg:flex flex-col bg-white border-r border-gray-200 sticky top-16 h-[calc(100vh-4rem)] transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'
                    }`}
            >
                {/* Collapse Toggle */}
                ...
                <div className="flex justify-around items-center py-2">
                    {navItems.slice(0, 5).map((item) => (
                        <Link
                            key={item.path}
                            to={item.path}
                            className={`flex flex-col items-center gap-1 px-3 py-2 rounded-lg transition-colors ${isActive(item.path)
                                ? 'text-blue-600'

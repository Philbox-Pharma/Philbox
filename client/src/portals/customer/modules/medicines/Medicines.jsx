...
export default function Medicines() {
  ...
  const filteredMedicines = medicines.filter(med => {
    ...
    if (
      ...
      med.brand !== filters.brand
    )
      return false;
    if (filters.prescriptionRequired === 'yes' && !med.prescriptionRequired)
      return false;
    if (filters.prescriptionRequired === 'no' && med.prescriptionRequired)
      return false;
    if (
      searchQuery &&
                ...
                <div className="relative">
                  ...
                  {medicine.originalPrice > medicine.price && (
                    <span className="absolute top-2 right-2 bg-green-500 text-white text-xs px-2 py-1 rounded">
                      {Math.round(
                        (1 - medicine.price / medicine.originalPrice) * 100
                      )}
                      % OFF
                    </span>
                  )}

...
import { adminAuthApi, activityLogsApi } from '../../../../core/api/admin/adminApi';
...
export default function AdminHeader({ toggleSidebar, sidebarOpen, admin }) {
    ...
    const fetchNotifications = async () => {
        ...
        try {
            ...
            // Fallback mock data
            setNotifications([
                { id: 1, text: 'No recent notifications', time: 'Just now', type: 'info', unread: false }
            ]);
            setUnreadCount(0);
        } finally {
    ...
    const getNotificationType = (actionType) => {
        ...
        if (actionType.includes('create') || actionType.includes('add')) return 'success';
        if (actionType.includes('delete') || actionType.includes('remove')) return 'danger';
    ...
    const adminCategory = admin?.category || admin?.admin_category || 'super-admin';
    const displayCategory = adminCategory === 'super-admin' || adminCategory === 'super_admin'
        ? 'Super Admin'
        : 'Branch Admin';
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative" ref={profileRef}>
                        ...
                        >
                            ...
                            <FaChevronDown className={`text-sm transition-transform hidden sm:block ${profileOpen ? 'rotate-180' : ''}`} />
                        </button>
                                    ...
                                    <div className="p-4 bg-gradient-to-r from-[#1a365d] to-[#2c5282] text-white">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-sm text-white/80 truncate">{adminEmail}</p>
                                            </div>
                                        </div>
                                        <div className="mt-2">

...
export default function AdminList() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value, row) => (
        <span
          className={`px-3 py-1 rounded-full text-xs font-medium ${getRoleBadge(row)}`}
        >
    ...
    },
    {
      ...
      render: (value, row) => {
        ...
        return (
          <span
            className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${getStatusBadge(status)}`}
          >
            {status}
          </span>
        );
  ...
  const mobileCardRender = row => {
    ...
    return (
      <div className="p-4">
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span
                className={`inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-medium ${getRoleBadge(row)}`}
              >
                {isSuperAdmin(row) ? 'Super Admin' : 'Branch Admin'}
              </span>
            </div>
          ...
          <span
            className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${getStatusBadge(status)}`}
          >
            {status}
          </span>
        </div>
        ...
        {/* Actions */}
        <div className="flex items-center justify-between sm:justify-end gap-2 mt-4 pt-3 border-t border-gray-100">
          <Link
            to={`/admin/staff/admins/${row._id}`}

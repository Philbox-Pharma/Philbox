...
import {
    FaArrowLeft, FaSave, FaUserShield, FaEnvelope, FaPhone,
    FaSpinner, FaCamera, FaTrash, FaCodeBranch, FaExclamationTriangle
} from 'react-icons/fa';
...
export default function EditAdmin() {
    ...
    useEffect(() => {
        const fetchBranches = async () => {
            ...
            try {
                ...
                setBranchOptions([
                    { value: '507f1f77bcf86cd799439011', label: 'Lahore Main Branch (PHIL25#001)' },
                    { value: '507f1f77bcf86cd799439012', label: 'Karachi Branch (PHIL25#002)' }
                ]);
            } finally {
    ...
    const fetchAdminData = async () => {
        ...
        try {
            ...
            if (response.status === 200 || response.data) {
                ...
                const adminPhone = admin.phone_number || admin.contactNumber || admin.phone || '';
                const adminStatus = admin.status || admin.account_status || 'active';
                const adminCategory = admin.category || admin.admin_category || 'branch-admin';
                const adminProfileImg = admin.profile_img_url || admin.profileImg || admin.avatar || null;
                const adminCoverImg = admin.cover_img_url || admin.coverImg || null;
                const adminBranches = admin.branches_managed?.map(b => b._id || b) || [];
    ...
    const validationRules = {
        name: {
            required: true,
            minLength: 3,
            message: { required: 'Name is required', minLength: 'Minimum 3 characters' }
        }
    };
    ...
    const handleCoverImgChange = (e) => {
        ...
        if (file) {
            ...
            if (!file.type.startsWith('image/')) {
                setErrors(prev => ({ ...prev, coverImg: 'Please select an image file' }));
                return;
            }
            setCoverImg(file);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const branchesChanged = JSON.stringify([...formData.branches_managed].sort()) !==
            ...
            if (err.data?.error && Array.isArray(err.data.error)) {
                ...
                setErrors(prev => ({ ...prev, ...backendErrors, submit: 'Please fix the errors below' }));
            } else {
                setErrors(prev => ({ ...prev, submit: err.message || 'Failed to update admin' }));
            }
        ...
        return (
            <div className="space-y-4">
                <div className="flex items-center gap-4">
                    <Link to="/admin/staff/admins" className="p-2 hover:bg-gray-100 rounded-lg">
                        <FaArrowLeft />
                    </Link>
    ...
    return (
        <div className="space-y-6">
            {/* Header */}
            <div className="flex items-center gap-4">
                <Link to={`/admin/staff/admins/${id}`} className="p-2 hover:bg-gray-100 rounded-lg">
                    <FaArrowLeft />
                </Link>
                <div className="min-w-0">
                    <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Edit Admin</h1>
            ...
            >
                ...
                <div className="p-4 sm:p-6 space-y-8">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-4 sm:gap-6">
                            <div className="relative">
                                <div className={`w-20 h-20 sm:w-24 sm:h-24 rounded-xl border-4 border-white shadow-lg overflow-hidden flex items-center justify-center ${isSuperAdmin ? 'bg-purple-100' : 'bg-blue-100'
                                    }`}>
                                    {profilePreview ? (
                                        <img
                                            src={profilePreview}
                                        ...
                                        <FaUserShield className={`text-3xl sm:text-4xl ${isSuperAdmin ? 'text-purple-400' : 'text-blue-400'}`} />
                                    )}
                                </div>
                            ...
                            <div>
                                ...
                                {errors.profileImg && (
                                    <p className="text-red-500 text-xs mt-1">{errors.profileImg}</p>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    {formData.category !== originalData?.category && (
                        <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                            <p className="text-purple-800 text-sm">
                                <strong>Warning:</strong> Changing category from "{originalData?.category === 'super-admin' ? 'Super Admin' : 'Branch Admin'}" to "{formData.category === 'super-admin' ? 'Super Admin' : 'Branch Admin'}" will {formData.category === 'super-admin'
                    ...
                    {formData.status !== 'active' && (
                        <div className={`border rounded-lg p-4 ${formData.status === 'blocked'
                                ? 'bg-red-50 border-red-200'
                                : 'bg-yellow-50 border-yellow-200'
                            }`}>
                            <p className={`text-sm ${formData.status === 'blocked' ? 'text-red-800' : 'text-yellow-800'
                                }`}>
                                <strong>Warning:</strong> Changing status to "{formData.status}" will {formData.status === 'suspended'

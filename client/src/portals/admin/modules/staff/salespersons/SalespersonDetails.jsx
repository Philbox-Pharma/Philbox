...
import {
    ...
    FaCalendar, FaSpinner, FaCodeBranch, FaVenusMars,
    FaBirthdayCake, FaIdCard, FaToggleOn, FaToggleOff, FaCheckCircle,
    FaTasks, FaClipboardList, FaExclamationTriangle, FaBan
} from 'react-icons/fa';
...
export default function SalespersonDetails() {
        ...
        try {
            ...
            setPerson({
                ...
                branches_to_be_managed: [
                    { _id: '1', name: 'Lahore Main Branch', code: 'PHIL25#001', status: 'Active' }
                ],
    ...
    const getStatusModalConfig = () => {
        switch (newStatus) {
            case 'active':
                return { title: 'Activate Salesperson', type: 'success', confirmText: 'Activate' };
            case 'suspended':
                return { title: 'Suspend Salesperson', type: 'warning', confirmText: 'Suspend' };
            case 'blocked':
                return { title: 'Block Salesperson', type: 'danger', confirmText: 'Block' };
            default:
                return { title: 'Change Status', type: 'warning', confirmText: 'Confirm' };
        }
    };
    ...
    if (error && !person) {
        return (
            <div className="text-center p-12">
                ...
                <Link to="/admin/staff/salespersons" className="text-[#1a365d] hover:underline mt-4 inline-block">
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-4">
                    <Link to="/admin/staff/salespersons" className="p-2 hover:bg-gray-100 rounded-lg">
                        <FaArrowLeft />
                    </Link>
                    <div className="flex-1 min-w-0">
                        <h1 className="text-xl sm:text-2xl font-bold text-gray-800 truncate">{person?.fullName}</h1>
    ...
    <div className="h-5=10 sm:h-13 bg-gradient-to-r from-orange-500 to-orange-600 relative">
        <div className="absolute top-3 right-3">
            <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase shadow-sm ${getStatusBadge(person?.status)}`}>
                {person?.status}
            </span>
        </div>
    </div>
    ...
    <div className="px-4 sm:px-6 pb-6">
        ...
        <div className="flex items-end sm:items-center gap-4 -mt-0.1">
            ...
            <div className="pb-1 sm:pb-0">
                <h2 className="text-lg sm:text-xl font-bold text-gray-800">{person?.fullName}</h2>
                <p className="text-gray-500 text-sm">Salesperson</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mt-6">
                        ...
                        <InfoCard icon={FaPhone} label="Phone" value={person?.contactNumber || 'Not set'} />
                        <InfoCard icon={FaVenusMars} label="Gender" value={person?.gender || 'Not set'} />
                        <InfoCard
                            icon={FaBirthdayCake}
                            label="Date of Birth"
                            value={person?.dateOfBirth ? new Date(person.dateOfBirth).toLocaleDateString() : 'Not set'}
                        />
                        <InfoCard
                            icon={FaCalendar}
                            label="Joined"
                        ...
                        <InfoCard
                            ...
                            valueClass={person?.isTwoFactorEnabled ? 'text-green-600' : 'text-gray-500'}
                        />
                    </div>
                </div>
            </div>
                                ...
                                >
                                    <div className="flex items-center gap-3 min-w-0">
                                        ...
                                        <div className="min-w-0">
                                            <p className="font-medium text-gray-800 truncate">{branch.name}</p>
                    ...
                    {taskLoading ? (
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4">
                                ...
                                />
                                <StatBox
                                    value={taskPerformance.metrics.statusBreakdown?.in_progress || 0}
                                    label="In Progress"
                                    color="yellow"
                                />
                                <StatBox
                            ...
                            <div className="mb-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    <span className="px-2 py-1 bg-gray-100 text-gray-600 rounded text-xs">
                                        Pending: {taskPerformance.metrics.statusBreakdown?.pending || 0}
                                    </span>
                                    <span className="px-2 py-1 bg-blue-100 text-blue-600 rounded text-xs">
                                        In Progress: {taskPerformance.metrics.statusBreakdown?.in_progress || 0}
                                    </span>
                                    <span className="px-2 py-1 bg-green-100 text-green-600 rounded text-xs">
                                        Completed: {taskPerformance.metrics.statusBreakdown?.completed || 0}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="p-3 bg-gray-50 rounded-lg">
                                ...
                                <p className="font-semibold text-gray-800">{taskPerformance.metrics.averageCompletionDays} days</p>
                            </div>
                        </>
                    ) : (
                        <div className="text-center py-8 text-gray-400">
...
// Sub-components
const InfoCard = ({ icon: Icon, label, value, valueClass = '', className = '' }) => (
    <div className={`flex items-center gap-3 ${className}`}>
        ...
        <div className="min-w-0 flex-1">
            ...
            <p className={`font-medium text-gray-800 truncate ${valueClass}`}>{value}</p>
        </div>
    </div>
);
...
const StatBox = ({ value, label, color }) => {
    ...
    return (
        <div className={`p-3 rounded-lg text-center ${colorMap[color]}`}>
            <p className={`text-xl sm:text-2xl font-bold ${textColor[color]}`}>{value}</p>

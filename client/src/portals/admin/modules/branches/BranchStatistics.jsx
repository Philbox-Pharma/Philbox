...
import {
    ...
    FaChartLine, FaMoneyBillWave, FaUsers, FaUndo, FaCalendarAlt, FaSearch,
...
export default function BranchStatistics() {
    ...
    // Branch Stats
    const [branchStats, setBranchStats] = useState({ total: 0, active: 0, inactive: 0 });
    ...
    const fetchRevenueData = async () => {
        ...
        try {
            ...
            const [trendsRes, splitRes, topRes, refundsRes, avgRes, paymentRes] = await Promise.all([
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold text-gray-800">Branch Statistics & Revenue Analytics</h1>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard icon={FaCodeBranch} label="Total Branches" value={branchStats.total} color="text-[#1a365d]" bg="bg-[#1a365d]/10" />
                <StatCard icon={FaCheckCircle} label="Active Branches" value={branchStats.active} color="text-green-600" bg="bg-green-100" />
                <StatCard icon={FaTimesCircle} label="Inactive Branches" value={branchStats.inactive} color="text-gray-500" bg="bg-gray-100" />
            </div>
                                ...
                                {topBranches.length > 0 ? (
                                    <ResponsiveContainer width="100%" height={250}>
                                        <BarChart data={topBranches} layout="vertical">
                                            ...
                                            <YAxis type="category" dataKey="branchName" tick={{ fontSize: 11 }} width={100} />
                                            <Tooltip formatter={(value) => `Rs. ${value.toLocaleString()}`} />
                                            <Bar dataKey="totalRevenue" name="Revenue" fill={COLORS.primary} radius={[0, 4, 4, 0]} />
                                        </BarChart>
                                    </ResponsiveContainer>
                                ) : (
        ...
        <div className="p-4 max-h-80 overflow-y-auto space-y-3">
            {branches.length > 0 ? branches.map(b => (
                ...
                >
                    ...
                    <span className={`text-xs self-center px-2 py-1 rounded bg-${color}-100 text-${color}-700`}>
                        {b.status}
                    </span>
                </Link>
            )) : (

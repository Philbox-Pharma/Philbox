...
export default function EditBranch() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (branchRes.success || branchRes.status === 200) {
          ...
          const adminIds =
            branch.under_administration_of?.map(a =>
              typeof a === 'object' ? a._id : a
            ) || [];
          const spIds =
            branch.salespersons_assigned?.map(s =>
          ...
          setFormData({
            ...
            google_map_link:
              address.google_map_link || branch.google_map_link || '',
  ...
  const validationRules = {
    ...
    city: {
      required: true,
      minLength: 2,
      message: { required: 'City is required' },
    },
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (!payload.under_administration_of?.length)
        delete payload.under_administration_of;
      if (!payload.salespersons_assigned?.length)
        delete payload.salespersons_assigned;
      if (!payload.phone) delete payload.phone;
      if (!payload.google_map_link) delete payload.google_map_link;

      console.log('Sending Payload:', payload);

      const response = await branchesService.update(id, payload); // Use .create() in AddBranch
        ...
        <div className="p-6 space-y-8">
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <FaCodeBranch className="text-[#1a365d]" /> Branch Info
            </h2>
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <FaMapMarkerAlt className="text-[#1a365d]" /> Address
            </h2>
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <FaUserTie className="text-[#1a365d]" /> Assign Staff
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormMultiSelect
                label="Assign Admins"
              ...
              <div className="flex gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder="Paste link or select from map..."
                    className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#1a365d] outline-none"
                  />
                </div>
                <button
                  type="button"
                  onClick={() => setMapOpen(true)}
                  className="px-4 py-2 bg-[#1a365d] text-white rounded-lg hover:bg-[#2c5282] flex items-center gap-2 whitespace-nowrap"
                >
                ...
                <div className="rounded-lg overflow-hidden border border-gray-200 mt-3 h-64 bg-gray-100">
                  {getMapEmbedUrl(formData.google_map_link) ? (
                    <iframe
                      ...
                      style={{ border: 0 }}
                      loading="lazy"
                      src={getMapEmbedUrl(formData.google_map_link)}
                    />
                  ) : (
                    <div className="h-full flex items-center justify-center text-gray-400">
                      <FaMapMarkerAlt className="mr-2" /> Preview unavailable
                    </div>
                  )}
                </div>
              )}
